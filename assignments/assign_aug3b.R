...

{
  ...
  ttheat=rep(0, times=12);
  ttcool=rep(0, times=12);
...
for(i in 1:nrow(weatherData))
  ...
  for(j in 1:length(Months))
    {
    if(weatherData$month[i]==Months[j]) 
    {
      ttheat[j] = ttheat[j] + weatherData$heatDays[i];
      ttcool[j]=  ttcool[j] + weatherData$coolDays[i];
      break;  # break out of the for loop (don't need to check other months)
    }
  }
}
#^^ do not know why my for loop isnt working and giving me an error that has to do with the }  
  # 
  #   3) Combine the three vectors into the heatCoolByMonth dataframe
  #    - make sure you name the columns appropriately
  newdf=data.frame(Months, ttheat, ttcool);
  ...
  firstc=rep(Months, times=2);
  sc1=rep("heatDays", times=12);
  sc2=rep("coolDays", times=12);
  secondc=c(sc1, sc2);
  thirdc=c(ttheat, ttcool);
  melted_DF=data.frame(firstc, secondc, thirdc);
  colnames(melted_DF)=c("month","variable","days");
  
  #   5) Extra: add an unofficial legend to the geom_col() plots using annotate()
  #    - the options for geom are: segment, rect, and text
  myplot = ggplot(melted_DF) +
    geom_col(mapping=aes(x=month, y=days, fill=variable),
             width=1,
             position=position_dodge())  +
    scale_x_discrete(limits = month.abb) +
    scale_fill_manual(values=c("aquamarine", "darkred")) +
    theme_bw() +
