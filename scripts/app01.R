#Connie Rojas
##Script for Application #1
#Task: to change the color of the boxplots (lines 21-24)

{
  ...
  thePlot = ggplot(data=weatherData) +
    geom_boxplot(mapping=aes(x=windDir, y=changeMaxTemp), 
                 na.rm=TRUE,
                 color=c("darkcyan", rep("gray21", 3),
                         "deeppink3", rep("gray21", 3),
                         "darkviolet", rep("gray21", 3)),
                 fill=c(rep(NA, 8), rep("goldenrod1", 3), NA)) +
    theme_bw() +
